...
export default function CreateRequest() {
  ...
  const [form, setForm] = useState({
    title: '', description: '', type: 'code_review',
    team: '', priority: 'medium', repo_url: '', tag_ids: [],
  });
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        <h1 className="font-display font-bold text-xl text-ink-50">New Request</h1>
        <p className="text-ink-400 text-sm mt-0.5">Post a mentorship or code review request for the team.</p>
      </div>
      ...
      <form onSubmit={submit} className="space-y-4">
        ...
        <div>
          ...
          <input className="input" placeholder="e.g. Review my auth middleware for security holes" value={form.title} onChange={set('title')} />
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Describe what you need help with, context, links, etc."
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="section-label block mb-1.5">Team / Dept *</label>
            <input className="input" placeholder="e.g. Platform, Growth" value={form.team} onChange={set('team')} />
          </div>
          <div>
        ...
        {/* Repo URL */}
        <div>
          <label className="section-label block mb-1.5">Repo / PR URL (optional)</label>
          <input className="input font-mono text-xs" placeholder="https://github.com/…" value={form.repo_url} onChange={set('repo_url')} />
        </div>
            ...
            <div className="flex flex-wrap gap-2">
              {tags.map(t => (
                <TagChip
                  key={t.id}
                  name={t.name}
                  active={form.tag_ids.includes(t.id)}
